...
mergedata <- paste(blog_sample[1:10000], news_sample[1:10000], twitter_sample[1:10000])
...
#dictionary.size.to.coverage <- cumsum(unigram$freq * 100 / sum(unigram$freq))
#plot(x=1:length(dictionary.size.to.coverage), y=dictionary.size.to.coverage, type="l", main="Coverage", xlab="Dictionary Size (words)", ylab="Coverage (percent)")

save(unigram, bigram, trigram,  file = "./ngrams.RData")
